...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry):
    ...
    for i, BOW in enumerate(BOWS):
        ...
        # Water Sensor Unit of Measure
        sensors = BOW["Sensor"]
        units = ""
        temp_unit = TEMP_CELSIUS
        
        for sensor in sensors:
            if sensor["Type"] == "SENSOR_WATER_TEMP":
                units = sensor["Units"]
        ...
        hass.states.async_set('omnilogic.%s_water_temp' %(bow_name), waterTemp, {"unit_of_measurement":temp_unit, "icon":"mdi:thermometer"})

        # Air Temp
        airTemp = telemetry_data['Backyard']['airTemp']
        hass.states.async_set('omnilogic.%s_air_temp' %(bow_name), airTemp, {"unit_of_measurement": temp_unit, "icon":"mdi:thermometer"})
        ...
        #omni_heater = OmniLogicHeater(heater_name, heater_minTemp, heater_maxTemp, heater_setPoint, temp_unit, heater_state, bow_systemId, heater_systemId)

        hass.states.async_set(
            f"omnilogic.{bow_name}_{heater_name}", heater_state, {"setpoint": heater_setPoint, "icon": "mdi:radiator","max_temp": heater_maxTemp, "min_temp": heater_minTemp, "heater_id": heater_systemId}
        )
